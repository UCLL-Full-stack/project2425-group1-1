...
const getAllSprints = (): Sprint[] => sprintDB.getAll();

const getSprintById = (id: number): Sprint => {
    const sprint = sprintDB.getById({ id });
...
const addBacklogItemsToSprint = (sprint_id: number, backlog_item_ids: number[]): BacklogItem[] => {
    const sprint = sprintDB.getById({ id: sprint_id });
    ...
    backlog_item_ids.forEach(backlog_item_id => {
        ...
        const backlogItem = backlogItemDB.getById({ id: backlog_item_id });
